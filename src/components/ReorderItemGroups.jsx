...
export const ReorderItemGroups = fastMemo(
  ({ itemGroups, renderItem, onReorder, onClickItem, reverse, height, bgColor, borderColor }) => {
    const containerRef = useRef(null);
    const contentRef = useRef(null);

    const [innerItemGroups, setInnerItemGroups] = useState(itemGroups);
    ...
    const contentHeight = useMemo(() => {
      const heights = innerItemGroups.map(group => {
    ...
    const handleDragMove = e => {
      if (dragging !== null) {
        ...
        const item = innerItemGroups[dragging.groupIndex].items[dragging.itemIndex];
        setPositions(prev => {
          return {
            ...prev,
            [item.sortKey]: {
    ...
    const reorderElements = draggedIndex => {
      const item = innerItemGroups[draggedIndex.groupIndex].items[draggedIndex.itemIndex];
      ...
      if (draggedIndex.groupIndex === closeResult.groupIndex) {
        newGroups = innerItemGroups.map((group, groupIndex) => {
    ...
    const findClosestIndex = (x, y, itemId) => {
      ...
      innerItemGroups.forEach((group, gIndex) => {
    ...
    useEffect(() => {
      ...
      setPositions(calculatePositions(itemGroups, iWidth));
      setInnerItemGroups(itemGroups);
    }, [itemGroups]);
    ...
    return (
      ...
      >
        ...
        >
          {!!innerItemGroups.length &&
            innerItemGroups.map((group, groupIndex) => (
  ...
  ({
    ...
    return items.map((item, index) => (
      ...
      >
        ...
        <span className="reorder-items-drag-content" onClick={() => onClickItem(item)}>
          {renderItem(item)}
        </span>
      </div>
    ));
...
function addItem(items, index, item) {
  const newItems = [];
  items.forEach((i, iIndex) => {
    if (iIndex === index) {
      newItems.push(item);
